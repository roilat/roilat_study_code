...
public class TreePanel extends JPanel {
     ...
     * @see cn.roilat.study.algorithm.basic.display.tree.TreePanel#CHILD_ALIGN_RELATIVE 
     * @see cn.roilat.study.algorithm.basic.display.tree.TreePanel#CHILD_ALIGN_ABSOLUTE 
     */
    ...
    public void drawAllNode(Node n, int x, Graphics g) {
        ...
        g.fillOval(x, y, gridWidth, gridHeight); //画结点的格子  

        g.setColor(stringColor);
        g.drawString(n.toString(), x + 10, fontY); //画结点的名字  

        if (n.hasChild()) {
            List<Node> c = n.getChilds();
            int tempPosx = childAlign == CHILD_ALIGN_RELATIVE
                ? x - (n.drawSize * (gridWidth + hGap) - hGap) / 2
                : x - (n.drawSize * (gridWidth + hGap) - hGap) / 2;

            int i = 0;
            for (Node node : c) {

                int newX = (i == 0) ? tempPosx : tempPosx + node.drawSize * (gridWidth + hGap) / 2; //孩子结点起始X  
                tempPosx += node.drawSize * (gridWidth + hGap);
                g.setColor(linkLineColor);
                g.drawLine(x + gridWidth / 2, y + gridHeight, newX + gridWidth / 2,
                    y + gridHeight + vGap); //画连接结点的线  
     ...
     * @param g 绘图上下文环境 
     */
    public void drawAllNode1(Node n, int x, Graphics g) {
        int y = n.getLayer() * (vGap + gridHeight) + startY;
        int fontY = y + gridHeight - 5; //5为测试得出的值，你可以通过FM计算更精确的，但会影响速度  
        ...
        if (n.hasChild()) {
            ...
            for (Node node : c) {
                ...
                g.drawLine(x + gridWidth / 2, y + gridHeight, newX + gridWidth / 2,
                    y + gridHeight + vGap); //画连接结点的线  
                drawAllNode1(node, newX, g);
                i++;
            }
        }
    }
    
    public static void printHeap(Integer[] array) {
