package cn.roilat.study.algorithm.distribution.raft;
...
public class CounterService implements StateMachine, RAFT.RoleChange {
	...
	public CounterService(Channel ch) {
		...
		raft.raftId(ch.getName()).addRoleListener(this);
	}
	...
	public void dumpLog() {
		...
		raft.logEntries((entry, index) -> {
			StringBuilder log = new StringBuilder().append(index).append(" (").append(entry.term()).append("): ");
			...
			try {
				...
				switch (cmd) {
				case CREATE:
					log.append(cmd).append("(").append(name).append(", ").append(Bits.readLong(in)).append(")");
					break;
				case GET:
				case INCREMENT_AND_GET:
				case DECREMENT_AND_GET:
					log.append(cmd).append("(").append(name).append(")");
					break;
				default:
	...
	public byte[] apply(byte[] data, int offset, int length) throws Exception {
		...
		String name = Bits.readAsciiString(in).toString();
		System.out
				.println("[" + new SimpleDateFormat("HH:mm:ss.SSS").format(new Date()) + "] Apply: cmd=[" + cmd + "]");
	...
	private Object invoke(Command cmd, String name, boolean ignoreRetVal, long... values) {
		...
		try {
			...
			throw new RuntimeException("Serialization failure (cmd=" + cmd + ", name=" + name + ")", ex);
		} catch (Exception ex) {
			throw new RuntimeException("Raft set failure (cmd=" + cmd + ", name=" + name + ")", ex);
		}
	}
