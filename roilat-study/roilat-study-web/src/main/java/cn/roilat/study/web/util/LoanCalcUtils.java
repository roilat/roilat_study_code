...
public class LoanCalcUtils {
	...
	/**
	 * 1、计算上月的当天到本月的当天共有多少天
	 * 2、由于天数都会小于28号，所以每天的最近一个月的时间都确定的
	 * 3、Calendar获取到的月份都从0（1月开始）
	 * 比如12月x号到1月x号之间的天数为12月的天数(x<28)
	 */
	...
	public static List<BigDecimal[]> calcForEqualCapitalPlusInterestByMonth(BigDecimal loanAmt, BigDecimal prftRat, int prodPerdSum) throws Exception {
		...
		BigDecimal t0 = prftRat.add(new BigDecimal(12));// B+12
		BigDecimal t1 = ArithUtil.pow(t0, prodPerdSum);//=(B+12)^m
		BigDecimal fz = loanAmt.multiply(prftRat).multiply(t1);//分子：AB(12+B)^m
		BigDecimal fm = t1.multiply(new BigDecimal(12)).add(new BigDecimal(Math.pow(12, prodPerdSum+1)).negate());//分母：12*(12+m)^m-12^(m+1)
		BigDecimal monthRepay = fz.divide(fm,6,BigDecimal.ROUND_HALF_UP);//fz/fm，月还款金额
		...
		while (prodPerdSum-- > 0) {
			...
			//result[1] = monthRepay.add(result[2].negate());//应还本金=月应还金额-应还利息
			...
			result[1] = result[0].add(result[2].negate());//四舍五入后计算（利息的误差通过求差来抵消）
			
			surplusAmt = surplusAmt.add(result[1].negate());//剩余本金=上次剩余本金-本次应还本金（两位小数）
			if(prodPerdSum == 0){//最后一个月
				result[0] = result[0].add(totalAmt.setScale(2,BigDecimal.ROUND_HALF_UP));//总金额误差加回来
				result[1] = result[1].add(surplusAmt);//将剩余的本金加进去
				result[2] = result[2].add(surplusAmt.negate()).add(totalAmt.setScale(2,BigDecimal.ROUND_HALF_UP));//将剩余的本金减去，保证本息和不变，还要加上变动的总金额
			}
			results.add(result);
		}
		return results;
	...
	public static List<BigDecimal[]> calcForEqualCapitalByMonth(BigDecimal loanAmt, BigDecimal prftRat, int prodPerdSum) throws Exception {
		...
		while (prodPerdSum-- > 0) {
			...
			result[2] = result[2].setScale(2,BigDecimal.ROUND_HALF_UP);//四舍五入
			result[0] = result[1].add(result[2]);//四舍五入后计算（利息的误差通过求和来抵消）

			surplusAmt = surplusAmt.add(result[1].negate());//剩余本金=上次剩余本金-本次应还本金(两位小数)
			if(prodPerdSum == 0){//最后一个月
				result[1] = result[1].add(surplusAmt.setScale(2,BigDecimal.ROUND_HALF_UP));//将剩余的本金误差
				result[2] = result[2].add(totalIntAmt.setScale(2,BigDecimal.ROUND_HALF_UP));//将剩余的利息误差加上
	...
	public static List<BigDecimal[]> calcForRepayInterestPerMonthByMonth(BigDecimal loanAmt, BigDecimal prftRat, int prodPerdSum) throws Exception {
		...
		while (prodPerdSum-- > 0) {
			...
			if (prodPerdSum == 0) {
				result[2] = result[2].add(totalIntAmt.setScale(2,BigDecimal.ROUND_HALF_UP));//将利息的误差加进去
			}
...
//		BigDecimal dayRat = prftRat.divide(new BigDecimal(365));// monthRat=B/12,月利率
		BigDecimal totalIntAmt = new BigDecimal("0");
		BigDecimal[] result;
		int i = 0;
		while (prodPerdSum > i) {
...
//		BigDecimal dayRat = prftRat.divide(new BigDecimal(365));// dayRat=B/365,日利率
		BigDecimal[] result;
		int i = 0;
		int totalDays = 0;
		while (prodPerdSum > i) {
	 ...
	 * @param prftRat		产品利率
	 * @param prodPerdSum	产品期限
	 * @throws Exception 
	 */
	@Deprecated
	public static BigDecimal[] calcTotalIntAndPple(BigDecimal invstAmt, String repayType, BigDecimal prftRat, int prodPerdSum) throws Exception {
		...
		if ("30000020".equals(repayType)) {
			BigDecimal t0 = prftRat.add(new BigDecimal(12));// B+12
			BigDecimal t1 = ArithUtil.pow(t0, prodPerdSum);//=(B+12)^m
			BigDecimal fz = invstAmt.multiply(prftRat).multiply(t1);//分子：AB(12+B)^m
			BigDecimal fm = t1.multiply(new BigDecimal(12)).add(new BigDecimal(Math.pow(12, prodPerdSum+1)).negate());//分母：12*(12+m)^m-12^(m+1)
			BigDecimal monthRepay = fz.divide(fm,2,BigDecimal.ROUND_HALF_UP);//fz/fm，月还款金额
			result[0] = invstAmt;
			result[1] = monthRepay.multiply(new BigDecimal(prodPerdSum));
			
		} else if ("30000021".equals(repayType)) {

		} else if ("30000022".equals(repayType)) {

		} else if ("30000023".equals(repayType)) {
	...
	public static void main(String[] args) throws Exception {
		List<BigDecimal[]> list = LoanCalcUtils.calcForEqualCapitalByMonth(new BigDecimal("10000"), new BigDecimal("0.10"), 12);
