    ...
    public void testExtractFromTestCreation() throws IOException {
        ExtRenderListener extRenderListener = new ExtRenderListener() {
            ...
            public void renderText(TextRenderInfo renderInfo) {
                System.out.printf(" renderText (%s) (%s) %s",toString(renderInfo.getFillColor()),toString(renderInfo.getStrokeColor()),renderInfo.getText());
                System.out.println();
            }
            ...
            public Path renderPath(PathPaintingRenderInfo renderInfo) {
                ...
                if ((renderInfo.getOperation() & PathPaintingRenderInfo.FILL) != 0) {
                    System.out.printf("FILL (%s) LINEWiDTH(%s) ", toString(graphicsState.getFillColor()),graphicsState.getLineWidth());
                    if ((renderInfo.getOperation() & PathPaintingRenderInfo.STROKE) != 0)
                        System.out.print("and ");
                }else if ((renderInfo.getOperation() & PathPaintingRenderInfo.STROKE) != 0) {
                    System.out.printf("STROKE (%s) ", toString(graphicsState.getStrokeColor()));
                }else {
                    System.out.printf("OTHER (%s) ", renderInfo.getOperation());
                }
                ...
                for (PathConstructionRenderInfo pathConstructionRenderInfo : pathInfos) {
                    switch (pathConstructionRenderInfo.getOperation()) {
                        ...
                        case PathConstructionRenderInfo.CURVE_123:
                            System.out.printf("curve123 %s ",
                                transform(ctm, pathConstructionRenderInfo.getSegmentData()));
                            break;
                        case PathConstructionRenderInfo.CURVE_13:
                            System.out.printf("curve13 %s ",
                                transform(ctm, pathConstructionRenderInfo.getSegmentData()));
                            break;
                        case PathConstructionRenderInfo.CURVE_23:
                            System.out.printf("curve23 %s ",
                                transform(ctm, pathConstructionRenderInfo.getSegmentData()));
                            break;
                        case PathConstructionRenderInfo.LINETO:
                            System.out.printf("line to %s ",
                                transform(ctm, pathConstructionRenderInfo.getSegmentData()));
                            break;
                        case PathConstructionRenderInfo.RECT:
                            System.out.printf("rectangle %s ",
                                transform(ctm, expandRectangleCoordinates(
                                    pathConstructionRenderInfo.getSegmentData())));
                            break;
                    }
                }
                System.out.println();
            ...
            List<Float> expandRectangleCoordinates(List<Float> rectangle) {
                ...
                return Arrays.asList(rectangle.get(0), rectangle.get(1),
            ...
            String toString(BaseColor baseColor) {
                if (baseColor == null)
                    return "DEFAULT";
                return String.format("%s,%s,%s", baseColor.getRed(), baseColor.getGreen(),
                    baseColor.getBlue());
            }

            GraphicsState getGraphicsState(PathPaintingRenderInfo renderInfo) throws NoSuchFieldException,
                                                                              SecurityException,
