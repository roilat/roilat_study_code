		...
		<tbody data-type="body">
			<!if(list.length>0) { !>
				...
				<tr class="gradeA <!=classItem!>">
					<td title="<!=item["nodeId"]!>"><!=item["nodeId"]!></td>
					<td title="<!=item["treeId"]!>"><!=item["treeId"]!></td>
					<td title="<!=item["nodeNo"]!>"><!=item["nodeNo"]!></td>
					<td title="<!=item["nodeValue"]!>"><!=item["nodeValue"]!></td>
					<td title="<!=item["keyId"]!>"><!=item["keyId"]!></td>
					<td>
						<a title="详情" data-value="<!=item["nodeId"]!>" class="detail">详情</a>
						<a title="修改" data-value="<!=item["nodeId"]!>" class="edit">修改</a>
						<a title="删除" data-value="<!=item["nodeId"]!>" class="delete">删除</a>
					</td>
				</tr>
			<!}!>
			<!}else{!>
		...
		$(function(){
			...
			$("#addBtn").click(function(){
				seajs.use("component/dialog", function (dialog) {
				    daysNodeDialog = new dialog({
				        "width" : 800,
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/daysNode/toAdd.do", // 内容
				        "title" : "添加信息" //标题
					});
				});
			});
			...
			$("#tbwrap").delegate(".detail", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    daysNodeDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/daysNode/view.do?nodeId="+nodeId, // 内容
			...
			$("#tbwrap").delegate(".edit", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    daysNodeDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/daysNode/toEdit.do?nodeId="+nodeId, // 内容
			...
			$("#tbwrap").delegate(".delete", "click", function () {
				var nodeId = $(this).data("value");
				if (confirm('确定删除？')) {
					$.post("${ctx}/daysNode/delete.do", {nodeId : nodeId}, function (result) {
			            if ("succ" == result) {
			            	alert("操作成功");
							// 刷新列表页面
			                top.location.replace("${ctx}/daysNode/list.do?ts="+new Date().getTime()); //如果操作成功则跳到列表界面 
							// 关闭弹窗窗口
	        ...
	        seajs.use(["component/table"], function (uiTable) {
		        $.post("${ctx}/daysNode/ajaxPage.do",
		        {
	                ...
	                table.getPager().bind("changePage", function (event, page, hander) {
	                	$.post("${ctx}/daysNode/ajaxPage.do", { page: page,  deptCnName: deptCnName, deptEnName: deptEnName },
	                    function (result) {
	                    	result = strToObj(result);
