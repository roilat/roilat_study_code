...
<body>
		...
		<tbody data-type="body">
			<!if(list.length>0) { !>
				...
				<tr class="gradeA <!=classItem!>">
					...
					<td title="<!=item["month"]!>"><!=item["month"]!></td>
					<td title="<!=item["isLock"]!>"><!=item["isLock"]!></td>
					<td title="<!=item["businessLine"]!>"><!=item["businessLine"]!></td>
					<td>
						<a title="详情" data-value="<!=item["year"]!>" class="detail">详情</a>
						<a title="修改" data-value="<!=item["year"]!>" class="edit">修改</a>
						<a title="删除" data-value="<!=item["year"]!>" class="delete">删除</a>
					</td>
				</tr>
			<!}!>
			<!}else{!>
	...
	<script type="text/javascript">
		var hqclcfTimelockDialog;
		$(function(){
			// 1、添加
			$("#addBtn").click(function(){
				seajs.use("component/dialog", function (dialog) {
				    hqclcfTimelockDialog = new dialog({
				        "width" : 800,
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/hqclcfTimelock/toAdd.do", // 内容
				        "title" : "添加信息" //标题
					});
				});
			});
			...
			$("#tbwrap").delegate(".detail", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    hqclcfTimelockDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/hqclcfTimelock/view.do?year="+year, // 内容
			...
			$("#tbwrap").delegate(".edit", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    hqclcfTimelockDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/hqclcfTimelock/toEdit.do?year="+year, // 内容
			...
			$("#tbwrap").delegate(".delete", "click", function () {
				var year = $(this).data("value");
				if (confirm('确定删除？')) {
					$.post("${ctx}/hqclcfTimelock/delete.do", {year : year}, function (result) {
			            if ("succ" == result) {
			            	alert("操作成功");
							// 刷新列表页面
			                top.location.replace("${ctx}/hqclcfTimelock/list.do?ts="+new Date().getTime()); //如果操作成功则跳到列表界面 
							// 关闭弹窗窗口
	...
	<script type="text/javascript">
		...
		function search() {
	        ...
	        seajs.use(["component/table"], function (uiTable) {
		        $.post("${ctx}/hqclcfTimelock/ajaxPage.do",
		        {
		        	page: 1
		        	// 其他查询条件
		        }, function (result) {
	                ...
	                table.getPager().bind("changePage", function (event, page, hander) {
	                	$.post("${ctx}/hqclcfTimelock/ajaxPage.do", { page: page,  deptCnName: deptCnName, deptEnName: deptEnName },
	                    function (result) {
	                    	result = strToObj(result);
