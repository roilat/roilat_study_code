		...
		<tbody data-type="body">
			<!if(list.length>0) { !>
				...
				<tr class="gradeA <!=classItem!>">
					<td title="<!=item["typeCode"]!>"><!=item["typeCode"]!></td>
					<td title="<!=item["typeName"]!>"><!=item["typeName"]!></td>
					<td title="<!=item["id"]!>"><!=item["id"]!></td>
					<td>
						<a title="详情" data-value="<!=item["typeCode"]!>" class="detail">详情</a>
						<a title="修改" data-value="<!=item["typeCode"]!>" class="edit">修改</a>
						<a title="删除" data-value="<!=item["typeCode"]!>" class="delete">删除</a>
					</td>
				</tr>
			<!}!>
			<!}else{!>
		...
		$(function(){
			...
			$("#addBtn").click(function(){
				seajs.use("component/dialog", function (dialog) {
				    bizTypeDialog = new dialog({
				        "width" : 800,
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/bizType/toAdd.do", // 内容
				        "title" : "添加业务参数类型设置信息" //标题
					});
				});
			});
			// 2、详细
			$("#tbwrap").delegate(".detail", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    bizTypeDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/bizType/view.do?typeCode="+typeCode, // 内容
			...
			$("#tbwrap").delegate(".edit", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    bizTypeDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/bizType/toEdit.do?typeCode="+typeCode, // 内容
			...
			$("#tbwrap").delegate(".delete", "click", function () {
				var typeCode = $(this).data("value");
				if (confirm('确定删除？')) {
					$.post("${ctx}/bizType/delete.do", {typeCode : typeCode}, function (result) {
			            if ("succ" == result) {
			            	alert("操作成功");
							// 刷新列表页面
			                top.location.replace("${ctx}/bizType/list.do?ts="+new Date().getTime()); //如果操作成功则跳到列表界面 
							// 关闭弹窗窗口
	        ...
	        seajs.use(["component/table"], function (uiTable) {
		        $.post("${ctx}/bizType/ajaxPage.do",
		        {
	                ...
	                table.getPager().bind("changePage", function (event, page, hander) {
	                	$.post("${ctx}/bizType/ajaxPage.do", { page: page,  deptCnName: deptCnName, deptEnName: deptEnName },
	                    function (result) {
	                    	result = strToObj(result);
