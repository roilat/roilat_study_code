...
<body>
		...
		<tbody data-type="body">
			<!if(list.length>0) { !>
				...
				<tr class="gradeA <!=classItem!>">
					...
					<td title="<!=item["queryTime"]!>"><!=item["queryTime"]!></td>
					<td title="<!=item["custName"]!>"><!=item["custName"]!></td>
					<td title="<!=item["custCertType"]!>"><!=item["custCertType"]!></td>
					<td title="<!=item["custCertNo"]!>"><!=item["custCertNo"]!></td>
					<td title="<!=item["mobileNo"]!>"><!=item["mobileNo"]!></td>
					<td title="<!=item["isTarget"]!>"><!=item["isTarget"]!></td>
	...
	<script type="text/javascript">
		var blklistQueryLogDialog;
		$(function(){
			// 1、添加
			$("#addBtn").click(function(){
				seajs.use("component/dialog", function (dialog) {
				    blklistQueryLogDialog = new dialog({
				        "width" : 800,
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/blklistQueryLog/toAdd.do", // 内容
				        "title" : "添加黑名单查询日志信息" //标题
					});
				});
			});
			// 2、详细
			$("#tbwrap").delegate(".detail", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    blklistQueryLogDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/blklistQueryLog/view.do?orgId="+orgId, // 内容
			...
			$("#tbwrap").delegate(".edit", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    blklistQueryLogDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/blklistQueryLog/toEdit.do?orgId="+orgId, // 内容
			...
			$("#tbwrap").delegate(".delete", "click", function () {
				var orgId = $(this).data("value");
				if (confirm('确定删除？')) {
					$.post("${ctx}/blklistQueryLog/delete.do", {orgId : orgId}, function (result) {
			            if ("succ" == result) {
			            	alert("操作成功");
							// 刷新列表页面
			                top.location.replace("${ctx}/blklistQueryLog/list.do?ts="+new Date().getTime()); //如果操作成功则跳到列表界面 
							// 关闭弹窗窗口
	...
	<script type="text/javascript">
		...
		function search() {
	        ...
	        seajs.use(["component/table"], function (uiTable) {
		        $.post("${ctx}/blklistQueryLog/ajaxPage.do",
		        {
		        	page: 1
		        	// 其他查询条件
		        }, function (result) {
	                ...
	                table.getPager().bind("changePage", function (event, page, hander) {
	                	$.post("${ctx}/blklistQueryLog/ajaxPage.do", { page: page,  deptCnName: deptCnName, deptEnName: deptEnName },
	                    function (result) {
	                    	result = strToObj(result);
