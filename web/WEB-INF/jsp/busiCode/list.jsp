		...
		<tbody data-type="body">
			<!if(list.length>0) { !>
				...
				<tr class="gradeA <!=classItem!>">
					<td title="<!=item["bizTypeCode"]!>"><!=item["bizTypeCode"]!></td>
					<td title="<!=item["bizCodeValue"]!>"><!=item["bizCodeValue"]!></td>
					<td title="<!=item["bizCodeName"]!>"><!=item["bizCodeName"]!></td>
					<td title="<!=item["id"]!>"><!=item["id"]!></td>
					<td>
						<a title="详情" data-value="<!=item["bizTypeCode"]!>" class="detail">详情</a>
						<a title="修改" data-value="<!=item["bizTypeCode"]!>" class="edit">修改</a>
						<a title="删除" data-value="<!=item["bizTypeCode"]!>" class="delete">删除</a>
					</td>
				</tr>
			<!}!>
			<!}else{!>
		...
		$(function(){
			...
			$("#addBtn").click(function(){
				seajs.use("component/dialog", function (dialog) {
				    busiCodeDialog = new dialog({
				        "width" : 800,
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/busiCode/toAdd.do", // 内容
				        "title" : "添加业务参数设置信息" //标题
					});
				});
			});
			...
			$("#tbwrap").delegate(".detail", "click", function () {
				// 2.1 获取主键
				var bizTypeCode = $(this).data("value");
				// 2.2 弹窗窗口，显示详细信息
				seajs.use("component/dialog", function (dialog) {
				    busiCodeDialog = new dialog({
				        "width" : 800,
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/busiCode/view.do?bizTypeCode="+bizTypeCode, // 内容
			...
			$("#tbwrap").delegate(".edit", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    busiCodeDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/busiCode/toEdit.do?bizTypeCode="+bizTypeCode, // 内容
			...
			$("#tbwrap").delegate(".delete", "click", function () {
				var bizTypeCode = $(this).data("value");
				if (confirm('确定删除？')) {
					$.post("${ctx}/busiCode/delete.do", {bizTypeCode : bizTypeCode}, function (result) {
			            if ("succ" == result) {
			            	alert("操作成功");
							// 刷新列表页面
			                top.location.replace("${ctx}/busiCode/list.do?ts="+new Date().getTime()); //如果操作成功则跳到列表界面 
							// 关闭弹窗窗口
			    			parent.busiCodeDialog.close();
			            } else {
			            	alert("操作失败");
			            }
			        }, "json");
	        ...
	        seajs.use(["component/table"], function (uiTable) {
		        $.post("${ctx}/busiCode/ajaxPage.do",
		        {
	                ...
	                table.getPager().bind("changePage", function (event, page, hander) {
	                	$.post("${ctx}/busiCode/ajaxPage.do", { page: page,  deptCnName: deptCnName, deptEnName: deptEnName },
	                    function (result) {
	                    	result = strToObj(result);
