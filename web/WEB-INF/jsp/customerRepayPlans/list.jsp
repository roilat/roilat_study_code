...
<body>
	...
	<div>
		...
		<thead>
			<tr>
				...
				<th>年化收益</th>
				<th>还款日期</th>
				<th>还款方式:0-等额本息;1-等额本金;2-按天计息，按月付息，到期还本;3-到期还本还息</th>
				<th>还款期数</th>
				<th>计划付款日期</th>
				<th>付款天数</th>
				<th>计划付款金额</th>
		...
		<tbody data-type="body">
			<!if(list.length>0) { !>
				...
				<tr class="gradeA <!=classItem!>">
					...
					<td title="<!=item["payPlanDt"]!>"><!=item["payPlanDt"]!></td>
					<td title="<!=item["payDays"]!>"><!=item["payDays"]!></td>
					<td title="<!=item["payPlanAmt"]!>"><!=item["payPlanAmt"]!></td>
					<td title="<!=item["payPlanIntAmt"]!>"><!=item["payPlanIntAmt"]!></td>
					<td title="<!=item["payPlanPpleAmt"]!>"><!=item["payPlanPpleAmt"]!></td>
					<td title="<!=item["payActualDt"]!>"><!=item["payActualDt"]!></td>
					<td title="<!=item["payActualAmt"]!>"><!=item["payActualAmt"]!></td>
					<td title="<!=item["payActualIntAmt"]!>"><!=item["payActualIntAmt"]!></td>
					<td title="<!=item["payActualPpleAmt"]!>"><!=item["payActualPpleAmt"]!></td>
					<td title="<!=item["stat"]!>"><!=item["stat"]!></td>
					<td title="<!=item["created"]!>"><!=item["created"]!></td>
					<td title="<!=item["modified"]!>"><!=item["modified"]!></td>
					<td>
						<a title="详情" data-value="<!=item["id"]!>" class="detail">详情</a>
						<a title="修改" data-value="<!=item["id"]!>" class="edit">修改</a>
						<a title="删除" data-value="<!=item["id"]!>" class="delete">删除</a>
					</td>
				</tr>
			<!}!>
			<!}else{!>
	...
	<script type="text/javascript">
		var customerRepayPlansDialog;
		$(function(){
			// 1、添加
			$("#addBtn").click(function(){
				seajs.use("component/dialog", function (dialog) {
				    customerRepayPlansDialog = new dialog({
				        "width" : 800,
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/customerRepayPlans/toAdd.do", // 内容
				        "title" : "添加手机信息历史信息" //标题
					});
				});
			});
			...
			$("#tbwrap").delegate(".detail", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    customerRepayPlansDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/customerRepayPlans/view.do?id="+id, // 内容
			...
			$("#tbwrap").delegate(".edit", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    customerRepayPlansDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/customerRepayPlans/toEdit.do?id="+id, // 内容
			...
			$("#tbwrap").delegate(".delete", "click", function () {
				var id = $(this).data("value");
				if (confirm('确定删除？')) {
					$.post("${ctx}/customerRepayPlans/delete.do", {id : id}, function (result) {
			            if ("succ" == result) {
			            	alert("操作成功");
							// 刷新列表页面
			                top.location.replace("${ctx}/customerRepayPlans/list.do?ts="+new Date().getTime()); //如果操作成功则跳到列表界面 
							// 关闭弹窗窗口
			    			parent.customerRepayPlansDialog.close();
			            } else {
			            	alert("操作失败");
			            }
			        }, "json");
	...
	<script type="text/javascript">
		...
		function search() {
	        ...
	        seajs.use(["component/table"], function (uiTable) {
		        $.post("${ctx}/customerRepayPlans/ajaxPage.do",
		        {
		        	page: 1
		        	// 其他查询条件
		        }, function (result) {
	                ...
	                table.getPager().bind("changePage", function (event, page, hander) {
	                	$.post("${ctx}/customerRepayPlans/ajaxPage.do", { page: page,  deptCnName: deptCnName, deptEnName: deptEnName },
	                    function (result) {
	                    	result = strToObj(result);
