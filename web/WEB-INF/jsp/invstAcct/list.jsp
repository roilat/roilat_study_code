		...
		<tbody data-type="body">
			<!if(list.length>0) { !>
				...
				<tr class="gradeA <!=classItem!>">
					...
					<td title="<!=item["payPwd"]!>"><!=item["payPwd"]!></td>
					<td title="<!=item["acctTyp"]!>"><!=item["acctTyp"]!></td>
					<td title="<!=item["openOrg"]!>"><!=item["openOrg"]!></td>
					<td title="<!=item["subjNo"]!>"><!=item["subjNo"]!></td>
					<td title="<!=item["Bal"]!>"><!=item["Bal"]!></td>
					<td title="<!=item["avalBal"]!>"><!=item["avalBal"]!></td>
					<td title="<!=item["frzAmt"]!>"><!=item["frzAmt"]!></td>
					<td title="<!=item["invstAmt"]!>"><!=item["invstAmt"]!></td>
					<td title="<!=item["batFlag"]!>"><!=item["batFlag"]!></td>
					<td title="<!=item["acctStat"]!>"><!=item["acctStat"]!></td>
					<td title="<!=item["frzStat"]!>"><!=item["frzStat"]!></td>
					<td title="<!=item["openDate"]!>"><!=item["openDate"]!></td>
					<td title="<!=item["lastTxDate"]!>"><!=item["lastTxDate"]!></td>
					<td title="<!=item["lastDayPrft"]!>"><!=item["lastDayPrft"]!></td>
					<td title="<!=item["lastMthPrft"]!>"><!=item["lastMthPrft"]!></td>
					<td title="<!=item["lastYearPrft"]!>"><!=item["lastYearPrft"]!></td>
					<td title="<!=item["totlPrft"]!>"><!=item["totlPrft"]!></td>
					<td title="<!=item["totlInvst"]!>"><!=item["totlInvst"]!></td>
					<td title="<!=item["totlAmt"]!>"><!=item["totlAmt"]!></td>
					<td title="<!=item["totlPpleAmt"]!>"><!=item["totlPpleAmt"]!></td>
					<td title="<!=item["totlIntAmt"]!>"><!=item["totlIntAmt"]!></td>
					<td title="<!=item["baseTotlPrft"]!>"><!=item["baseTotlPrft"]!></td>
					<td title="<!=item["prftAvgRat"]!>"><!=item["prftAvgRat"]!></td>
					...
					<td>
						<a title="详情" data-value="<!=item["acctNo"]!>" class="detail">详情</a>
						<a title="修改" data-value="<!=item["acctNo"]!>" class="edit">修改</a>
						<a title="删除" data-value="<!=item["acctNo"]!>" class="delete">删除</a>
					</td>
				</tr>
			<!}!>
			<!}else{!>
		...
		$(function(){
			...
			$("#addBtn").click(function(){
				seajs.use("component/dialog", function (dialog) {
				    invstAcctDialog = new dialog({
				        "width" : 800,
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/invstAcct/toAdd.do", // 内容
				        "title" : "添加投资人信息表信息" //标题
					});
				});
			});
			...
			$("#tbwrap").delegate(".detail", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    invstAcctDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/invstAcct/view.do?acctNo="+acctNo, // 内容
			...
			$("#tbwrap").delegate(".edit", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    invstAcctDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/invstAcct/toEdit.do?acctNo="+acctNo, // 内容
			...
			$("#tbwrap").delegate(".delete", "click", function () {
				var acctNo = $(this).data("value");
				if (confirm('确定删除？')) {
					$.post("${ctx}/invstAcct/delete.do", {acctNo : acctNo}, function (result) {
			            if ("succ" == result) {
			            	alert("操作成功");
							// 刷新列表页面
			                top.location.replace("${ctx}/invstAcct/list.do?ts="+new Date().getTime()); //如果操作成功则跳到列表界面 
							// 关闭弹窗窗口
	...
	<script type="text/javascript">
		...
		function search() {
	        ...
	        seajs.use(["component/table"], function (uiTable) {
		        $.post("${ctx}/invstAcct/ajaxPage.do",
		        {
		        	page: 1
		        	// 其他查询条件
		        }, function (result) {
	                ...
	                table.getPager().bind("changePage", function (event, page, hander) {
	                	$.post("${ctx}/invstAcct/ajaxPage.do", { page: page,  deptCnName: deptCnName, deptEnName: deptEnName },
	                    function (result) {
	                    	result = strToObj(result);
