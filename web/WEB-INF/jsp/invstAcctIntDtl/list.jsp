...
<body>
	...
	<div>
		...
		<thead>
			<tr>
				...
				<th>estIncomeDt</th>
				<th>estIncomeAmt</th>
				<th>actualIncomeDt</th>
				<th>actualIncomeAmt</th>
				<th>paymentPpleAmt</th>
				<th>actualPpleAmt</th>
				<th>paymentIntAmt</th>
				<th>actualIntAmt</th>
				<th>txStat</th>
		...
		<tbody data-type="body">
			<!if(list.length>0) { !>
				...
				<tr class="gradeA <!=classItem!>">
					...
					<td title="<!=item["prftTyp"]!>"><!=item["prftTyp"]!></td>
					...
					<td title="<!=item["estIncomeDt"]!>"><!=item["estIncomeDt"]!></td>
					<td title="<!=item["estIncomeAmt"]!>"><!=item["estIncomeAmt"]!></td>
					<td title="<!=item["actualIncomeDt"]!>"><!=item["actualIncomeDt"]!></td>
					<td title="<!=item["actualIncomeAmt"]!>"><!=item["actualIncomeAmt"]!></td>
					<td title="<!=item["paymentPpleAmt"]!>"><!=item["paymentPpleAmt"]!></td>
					<td title="<!=item["actualPpleAmt"]!>"><!=item["actualPpleAmt"]!></td>
					<td title="<!=item["paymentIntAmt"]!>"><!=item["paymentIntAmt"]!></td>
					<td title="<!=item["actualIntAmt"]!>"><!=item["actualIntAmt"]!></td>
	...
	<script type="text/javascript">
		var invstAcctIntDtlDialog;
		$(function(){
			// 1、添加
			$("#addBtn").click(function(){
				seajs.use("component/dialog", function (dialog) {
				    invstAcctIntDtlDialog = new dialog({
				        "width" : 800,
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/invstAcctIntDtl/toAdd.do", // 内容
				        "title" : "添加投资预期收益表信息" //标题
					});
				});
			});
			// 2、详细
			$("#tbwrap").delegate(".detail", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    invstAcctIntDtlDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/invstAcctIntDtl/view.do?txSeqNo="+txSeqNo, // 内容
			...
			$("#tbwrap").delegate(".edit", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    invstAcctIntDtlDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/invstAcctIntDtl/toEdit.do?txSeqNo="+txSeqNo, // 内容
			...
			$("#tbwrap").delegate(".delete", "click", function () {
				var txSeqNo = $(this).data("value");
				if (confirm('确定删除？')) {
					$.post("${ctx}/invstAcctIntDtl/delete.do", {txSeqNo : txSeqNo}, function (result) {
			            if ("succ" == result) {
			            	alert("操作成功");
							// 刷新列表页面
			                top.location.replace("${ctx}/invstAcctIntDtl/list.do?ts="+new Date().getTime()); //如果操作成功则跳到列表界面 
							// 关闭弹窗窗口
	...
	<script type="text/javascript">
		...
		function search() {
	        ...
	        seajs.use(["component/table"], function (uiTable) {
		        $.post("${ctx}/invstAcctIntDtl/ajaxPage.do",
		        {
		        	page: 1
		        	// 其他查询条件
		        }, function (result) {
	                ...
	                table.getPager().bind("changePage", function (event, page, hander) {
	                	$.post("${ctx}/invstAcctIntDtl/ajaxPage.do", { page: page,  deptCnName: deptCnName, deptEnName: deptEnName },
	                    function (result) {
	                    	result = strToObj(result);
