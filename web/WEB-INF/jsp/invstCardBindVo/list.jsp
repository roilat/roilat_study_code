...
<body>
		...
		<tbody data-type="body">
			<!if(list.length>0) { !>
				...
				<tr class="gradeA <!=classItem!>">
					...
					<td title="<!=item["provCod"]!>"><!=item["provCod"]!></td>
					<td title="<!=item["bankProv"]!>"><!=item["bankProv"]!></td>
					<td title="<!=item["cityCod"]!>"><!=item["cityCod"]!></td>
					<td title="<!=item["bankCity"]!>"><!=item["bankCity"]!></td>
					...
					<td>
						<a title="详情" data-value="<!=item["cardNo"]!>" class="detail">详情</a>
						<a title="修改" data-value="<!=item["cardNo"]!>" class="edit">修改</a>
						<a title="删除" data-value="<!=item["cardNo"]!>" class="delete">删除</a>
					</td>
				</tr>
			<!}!>
			<!}else{!>
	...
	<script type="text/javascript">
		var invstCardBindVoDialog;
		$(function(){
			// 1、添加
			$("#addBtn").click(function(){
				seajs.use("component/dialog", function (dialog) {
				    invstCardBindVoDialog = new dialog({
				        "width" : 800,
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/invstCardBindVo/toAdd.do", // 内容
				        "title" : "添加投资人银行卡绑定信息" //标题
					});
				});
			});
			// 2、详细
			$("#tbwrap").delegate(".detail", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    invstCardBindVoDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/invstCardBindVo/view.do?cardNo="+cardNo, // 内容
			...
			$("#tbwrap").delegate(".edit", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    invstCardBindVoDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/invstCardBindVo/toEdit.do?cardNo="+cardNo, // 内容
			...
			$("#tbwrap").delegate(".delete", "click", function () {
				var cardNo = $(this).data("value");
				if (confirm('确定删除？')) {
					$.post("${ctx}/invstCardBindVo/delete.do", {cardNo : cardNo}, function (result) {
			            if ("succ" == result) {
			            	alert("操作成功");
							// 刷新列表页面
			                top.location.replace("${ctx}/invstCardBindVo/list.do?ts="+new Date().getTime()); //如果操作成功则跳到列表界面 
							// 关闭弹窗窗口
			    			parent.invstCardBindVoDialog.close();
			            } else {
			            	alert("操作失败");
			            }
			        }, "json");
	...
	<script type="text/javascript">
		...
		function search() {
	        ...
	        seajs.use(["component/table"], function (uiTable) {
		        $.post("${ctx}/invstCardBindVo/ajaxPage.do",
		        {
		        	page: 1
		        	// 其他查询条件
		        }, function (result) {
	                ...
	                table.getPager().bind("changePage", function (event, page, hander) {
	                	$.post("${ctx}/invstCardBindVo/ajaxPage.do", { page: page,  deptCnName: deptCnName, deptEnName: deptEnName },
	                    function (result) {
	                    	result = strToObj(result);
