...
<body>
	<div>
		<form id="loanCustBaseInfoForm" action="post">
		<table class="table_list">
			...
			<tr>
				<td>手机号</td>
				<td><input type="text" id="mobileTel" name="mobileTel" data-validaterole="mobileTel" data-rule="mobileTel" value="" /></td>
			</tr>
			<tr>
				<td>贷款总额</td>
				<td><input type="text" id="totalLoanAmt" name="totalLoanAmt" data-validaterole="totalLoanAmt" data-rule="totalLoanAmt" value="" /></td>
			</tr>
			<tr>
				<td>最近贷款日期</td>
				<td><input type="text" id="maxLoanDate" name="maxLoanDate" data-validaterole="maxLoanDate" data-rule="maxLoanDate" value="" /></td>
			</tr>
			<tr>
				<td>信息来源公司</td>
				<td><input type="text" id="sourceCorp" name="sourceCorp" data-validaterole="sourceCorp" data-rule="sourceCorp" value="" /></td>
			</tr>
			<tr>
				<td colspan="2">
		...
		$(function(){
			...
			seajs.use(["component/validate","component/tip"], function (validate,tip) {
				...
				d.bind("onsubmit",function(){
					// 序列化表单参数
					var serializedForm = $("#loanCustBaseInfoForm").serialize();
					$.post("${ctx}/loanCustBaseInfo/add.do", serializedForm, function (result) {
			            if ("succ" == result) {
			            	alert("操作成功");
							// 刷新列表页面
			                top.location.replace("${ctx}/loanCustBaseInfo/list.do?ts="+new Date().getTime()); //如果发表成功则跳到列表界面 
							// 关闭弹窗窗口
