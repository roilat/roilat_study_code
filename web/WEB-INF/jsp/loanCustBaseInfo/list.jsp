...
<body>
		...
		<tbody data-type="body">
			<!if(list.length>0) { !>
				...
				<tr class="gradeA <!=classItem!>">
					...
					<td title="<!=item["mobileTel"]!>"><!=item["mobileTel"]!></td>
					<td title="<!=item["totalLoanAmt"]!>"><!=item["totalLoanAmt"]!></td>
					<td title="<!=item["maxLoanDate"]!>"><!=item["maxLoanDate"]!></td>
					<td title="<!=item["sourceCorp"]!>"><!=item["sourceCorp"]!></td>
					<td>
						<a title="详情" data-value="<!=item["custName"]!>" class="detail">详情</a>
						<a title="修改" data-value="<!=item["custName"]!>" class="edit">修改</a>
						<a title="删除" data-value="<!=item["custName"]!>" class="delete">删除</a>
					</td>
				</tr>
			<!}!>
			<!}else{!>
	...
	<script type="text/javascript">
		var loanCustBaseInfoDialog;
		$(function(){
			// 1、添加
			$("#addBtn").click(function(){
				seajs.use("component/dialog", function (dialog) {
				    loanCustBaseInfoDialog = new dialog({
				        "width" : 800,
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/loanCustBaseInfo/toAdd.do", // 内容
				        "title" : "添加贷款客户基本信息信息" //标题
					});
				});
			});
			// 2、详细
			$("#tbwrap").delegate(".detail", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    loanCustBaseInfoDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/loanCustBaseInfo/view.do?custName="+custName, // 内容
			...
			$("#tbwrap").delegate(".edit", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    loanCustBaseInfoDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/loanCustBaseInfo/toEdit.do?custName="+custName, // 内容
			...
			$("#tbwrap").delegate(".delete", "click", function () {
				var custName = $(this).data("value");
				if (confirm('确定删除？')) {
					$.post("${ctx}/loanCustBaseInfo/delete.do", {custName : custName}, function (result) {
			            if ("succ" == result) {
			            	alert("操作成功");
							// 刷新列表页面
			                top.location.replace("${ctx}/loanCustBaseInfo/list.do?ts="+new Date().getTime()); //如果操作成功则跳到列表界面 
							// 关闭弹窗窗口
			    			parent.loanCustBaseInfoDialog.close();
			            } else {
			            	alert("操作失败");
			            }
			        }, "json");
	...
	<script type="text/javascript">
		...
		function search() {
	        ...
	        seajs.use(["component/table"], function (uiTable) {
		        $.post("${ctx}/loanCustBaseInfo/ajaxPage.do",
		        {
		        	page: 1
		        	// 其他查询条件
		        }, function (result) {
	                ...
	                table.getPager().bind("changePage", function (event, page, hander) {
	                	$.post("${ctx}/loanCustBaseInfo/ajaxPage.do", { page: page,  deptCnName: deptCnName, deptEnName: deptEnName },
	                    function (result) {
	                    	result = strToObj(result);
