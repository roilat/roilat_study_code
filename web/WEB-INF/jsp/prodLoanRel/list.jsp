...
<body>
		...
		<tbody data-type="body">
			<!if(list.length>0) { !>
				...
				<tr class="gradeA <!=classItem!>">
					<td title="<!=item["prodTyp"]!>"><!=item["prodTyp"]!></td>
					<td title="<!=item["loanNo"]!>"><!=item["loanNo"]!></td>
					<td title="<!=item["redomNo"]!>"><!=item["redomNo"]!></td>
					<td title="<!=item["prodNo"]!>"><!=item["prodNo"]!></td>
					<td title="<!=item["prodName"]!>"><!=item["prodName"]!></td>
					<td title="<!=item["prodPerd"]!>"><!=item["prodPerd"]!></td>
					<td title="<!=item["loanCustNo"]!>"><!=item["loanCustNo"]!></td>
					<td title="<!=item["loanCustName"]!>"><!=item["loanCustName"]!></td>
					<td title="<!=item["marginAmt"]!>"><!=item["marginAmt"]!></td>
					<td title="<!=item["loanAmt"]!>"><!=item["loanAmt"]!></td>
					<td title="<!=item["actualAmt"]!>"><!=item["actualAmt"]!></td>
					<td title="<!=item["invstFlag"]!>"><!=item["invstFlag"]!></td>
					<td title="<!=item["instDate"]!>"><!=item["instDate"]!></td>
					<td title="<!=item["updtDate"]!>"><!=item["updtDate"]!></td>
					<td>
						<a title="详情" data-value="<!=item["prodTyp"]!>" class="detail">详情</a>
						<a title="修改" data-value="<!=item["prodTyp"]!>" class="edit">修改</a>
						<a title="删除" data-value="<!=item["prodTyp"]!>" class="delete">删除</a>
					</td>
				</tr>
			<!}!>
			<!}else{!>
	...
	<script type="text/javascript">
		var prodLoanRelDialog;
		$(function(){
			// 1、添加
			$("#addBtn").click(function(){
				seajs.use("component/dialog", function (dialog) {
				    prodLoanRelDialog = new dialog({
				        "width" : 800,
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/prodLoanRel/toAdd.do", // 内容
				        "title" : "添加产品与借款人关系表信息" //标题
					});
				});
			});
			// 2、详细
			$("#tbwrap").delegate(".detail", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    prodLoanRelDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/prodLoanRel/view.do?prodTyp="+prodTyp, // 内容
			...
			$("#tbwrap").delegate(".edit", "click", function () {
				...
				seajs.use("component/dialog", function (dialog) {
				    prodLoanRelDialog = new dialog({
				        ...
				        "height" : 600,
				        "contentType" : "iframe",
						"html" : "${ctx}/prodLoanRel/toEdit.do?prodTyp="+prodTyp, // 内容
			...
			$("#tbwrap").delegate(".delete", "click", function () {
				var prodTyp = $(this).data("value");
				if (confirm('确定删除？')) {
					$.post("${ctx}/prodLoanRel/delete.do", {prodTyp : prodTyp}, function (result) {
			            if ("succ" == result) {
			            	alert("操作成功");
							// 刷新列表页面
			                top.location.replace("${ctx}/prodLoanRel/list.do?ts="+new Date().getTime()); //如果操作成功则跳到列表界面 
							// 关闭弹窗窗口
	...
	<script type="text/javascript">
		...
		function search() {
	        ...
	        seajs.use(["component/table"], function (uiTable) {
		        $.post("${ctx}/prodLoanRel/ajaxPage.do",
		        {
		        	page: 1
		        	// 其他查询条件
		        }, function (result) {
	                ...
	                table.getPager().bind("changePage", function (event, page, hander) {
	                	$.post("${ctx}/prodLoanRel/ajaxPage.do", { page: page,  deptCnName: deptCnName, deptEnName: deptEnName },
	                    function (result) {
	                    	result = strToObj(result);
